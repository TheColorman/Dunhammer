const express = require('express'),
    ...
    redirect = encodeURIComponent('http://localhost:8081/old/api/discord/callback');
...
router.get('/login', catchAsync(async (req, res) => {
    if (req.signedCookies.access_token) {
        ...
        if (response.status != 401) {
            return res.send(`<p>You have been logged in, congratulations. Click <a href="/old/api/discord/logout">here</a> to log out or <a href="/old/">here</a> to go home.</p>`)
        }
    }
    let state;
...
router.get('/logout', (req, res) => {
    if (req.signedCookies.access_token) {
        ...
        res.end(`<p>You have been logged out.\n<a href="/old/">Go home.<a/></p>`);
    } else {
        res.write("<p>You were never logged in to begin with!</p>");
        res.end(`<p><a href="/old/">go home and sleep button that takes you home</a></p>`);
    }
})
...
router.get('/guilds', catchAsync(async (req, res) => {
    if (req.signedCookies.access_token) {
        const response = await fetch(`https://discord.com/api/v8/users/@me/guilds`, {
            ...
            names = filtered.map(element => {
                return element.name;
            });
        res.end(`You are admin in the following servers:\n${names.join("\n")}`);
    } else {
        res.end(`<p>It would seems you're not <a href="/old/api/discord/login">logged in</a></p>`);
    }
}));
