...
client.on("message", async (msg) => {
    ...
    if (level > user.level) {
        ...
        if ((reply.title + reply.description).includes("{")) {
            let title = reply.title.replace(/{/g, "[{").replace(/}/g, "}]").split(/\[(.*?)\]/);
            let description = reply.description.replace(/{/g, "[{").replace(/}/g, "}]").split(/\[(.*?)\]/);
            let index = 0;
            while (title[index] !== undefined) {
                index++;
            ...
            index = 0;
            while (description[index] !== undefined) {
                index++;
            ...
            reply.title = title.join('');
            reply.description = description.join('');
        }
